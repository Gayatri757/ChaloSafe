...
if df is not None:
    logging.info(f"✅ Crime data loaded: {len(df)} rows")
...
def calculate_risk_score(route_coords):
    """Calculate safety score (0–10) based on surrounding crime data."""
...
def recommend_route():
    ...
    try:
        ...
        for route in directions["features"]:
            ...
            safety_score = calculate_risk_score(coords)
            normalized_time = travel_time / 60  # minutes
            final_score = (0.9 * safety_score) + (0.1 * normalized_time)
